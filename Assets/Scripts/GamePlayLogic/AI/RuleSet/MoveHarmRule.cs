...
using Tactics.AI;
using UnityEngine;
public class MoveHarmRule : ScoreRuleBase
{
    public MoveHarmRule(DecisionSystem decisionSystem, List<IScoreRule> scoreSubRules, int scoreBonus, bool debugMode) : base(decisionSystem, scoreSubRules, scoreBonus, debugMode)
    {
    }
        ...
        GameNode moveNode, GameNode targetNode, int maxHealthAmongOpposites)
    {
        ...
        if (scoreSubRules != null && scoreSubRules.Count > 0)
        {
            foreach (var subRule in scoreSubRules)
                score += subRule.CalculateSkillScore(character, skill, targetNode, maxHealthAmongOpposites);
        }
