...
public class PlayerMoveTesting : MonoBehaviour
{
    ...
    private void UpAndDown(float xInput, float zInput)
    {
        ...
        if (DDADectector.DDARaycast(transform.position, direction, 1, world.loadedNodes,
            out Vector3Int? cubePosition1))
        {
            if (!isBusy)
            {
                Debug.Log(cubePosition1);
                float distance1 = Vector3.Distance(transform.position, cubePosition1.Value);
                Debug.Log(distance1);
                if (distance1 < 0.8f)
                {
